...
class Command(BaseCommand):
    help = (
        "Recompute needs_reorder for every Part with a reorder_point set, "
        "auto-draft purchase orders for newly-flagged parts, and email a digest."
    )

    def handle(self, *args, **options):
        parts = list(Parts.objects.filter(reorder_point__isnull=False))

        newly_flagged = []
        for part in parts:
            was_flagged = bool(part.needs_reorder)
            is_flagged = (part.on_hand or 0) <= part.reorder_point
        ...
        self.stdout.write(
            f"Scanned {len(parts)} part(s) with a reorder_point set; "
            f"{len(newly_flagged)} newly flagged for reorder."
        )
        ...
        if newly_flagged and settings.REORDER_ALERT_RECIPIENTS:
            lines = [
                ...
                f"on_hand={p.on_hand}, reorder_point={p.reorder_point}, "
                f"reorder_quantity={p.reorder_quantity}"
